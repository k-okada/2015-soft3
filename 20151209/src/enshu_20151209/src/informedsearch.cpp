...
class Problem {
 ...
 void readFile(string filename) {
   ...
   while (in2.getline(line, sizeof(line), '\n')) {
     ...
     if (line[0] != '#') {	
       ...
       while (((wordptr1 = strtok(NULL, delimiters)) != NULL) && ((wordptr2 = strtok(line, delimiters)) != NULL)){
         wordcount++;
 ...
 void testProblem() {
   ...
   addNode(string("Rimmicu-Vilcea"),193);
...
// sets up 1st dimension  with n() copies of dummy
   ...
   // edge between nodes and its cost
...
int main(int argc, char *argv[])
{
  int c;
  char *fname = NULL;
  enum { GS, AS } search_algorithm = GS;
