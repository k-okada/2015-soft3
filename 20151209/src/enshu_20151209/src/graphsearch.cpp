...
public:
    ...
    {  // add name to names only if it is
...
IDS_RESTART:
    ...
    while (!success) {
        ...
        cout << "considering " << problem.names[nodeptr->state] << endl;
        ...
        // DEPTH-TEST
        if ((search_algorithm == DLS || search_algorithm == IDS)
        ...
        if (recursive_check == false
            ...
            for (int j = 0; j < problem.nSuccessors(i); j++) {  // EXPAND
                Node* nextnode = Node::factory(problem.successors[i][j],
                    nodeptr);
                // diagnostics
                cout << "    "
    ...
    if (!success) {
        ...
        while (problem.names[nodeptr->state] != startName) {
            cout << problem.names[nodeptr->state] << "(depth " << nodeptr->depth << ")" << endl;
    ...
    cout << endl
         << Node::all.size() << " Nodes left" << endl;
    cout << endl
         << Node::count << " Nodes created" << endl;
}
